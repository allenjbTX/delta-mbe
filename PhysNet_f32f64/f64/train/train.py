...
parser.convert_arg_line_to_args = lambda arg_line: [arg_line] if arg_line.strip() else []
...
parser.add_argument('--show_progress', default=True, type=bool, help="show progress bar during training")
parser.add_argument('--use_shift', default=1, type=int, help="use trainable element-wise energy shift (set 0 for MBE n>=2 delta terms)")
...
def printProgressBar(iteration, total, prefix='', suffix='', decimals=1, length=100, fill='#', printEnd="\r"):
    ...
    print("\r{0} |{1}| {2}% {3}".format(prefix, bar, percent, suffix), end=printEnd, flush=True)
    if iteration == total:
        print(flush=True)
...
with tf.Session() as sess:
    ...
    while not coord.should_stop():
        ...
        if args.show_progress:
            if step == 1:
                time_step_estimation = (step_end - step_start) * (args.max_steps - step)
            else:
                time_step_estimation = (
                    0.5 * (time_step_estimation - (step_end - step_start))
                    + 0.5 * (step_end - step_start) * (args.max_steps - step))
            printProgressBar(
                step, args.max_steps,
                prefix="Step {0: 6d}/{1}".format(step, args.max_steps),
                suffix="Complete - Remaining Time: {0: 4.1f} s | Valid Loss: {1}     ".format(
                    time_step_estimation,
                    "{0: .4e}".format(last_valid_loss) if last_valid_loss is not None else "N/A"),
                length=42)
